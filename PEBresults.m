%This function shows the results of the DCM-PEB through a modified version of the script by Friston (spm_dcm_peb_review). 
%It requires two inputs: 1) a PEB (or a BMA) file; 2) A DCM (or a GCM) file. 
%Be careful because it will save the output images in the current
%directory.

function PEBresults(PEB, DCM)
...
thresholds = [0 0.5 0.75 0.90 0.95 0.99]; 
...
if display_threshold
    ...
    xPEB.Pp = Pp;
     ...
     %Apply lower threshold ADDED BY FB 25/03
     EpOrig = Ep;
        if ~isempty(Pp) && threshold > 0
        EpOrig = EpOrig .* (Pp(:) >= 0.75);
        end    
...
if display_connectivity            
    ...
    % Reshape PEB parameters also subthreshold (but higher than 0.75) ADDED
    % 25/03
    EqOrig = zeros(i,j,k);    
    for p = 1:np
        if strcmp(parts{p}.field, sel_field)
            EqOrig(parts{p}.row, parts{p}.col, parts{p}.input) = EpOrig(p);
        end
    end
    ...
    EqOrig = EqOrig(:,:,sel_input); %ADDED 25/03
    ...
    if display_connectivity
        ...
        if isempty(xPEB.mtx_fig) || ~ishandle(xPEB.mtx_fig)  
            ...
            xPEB.mtx_fig = figure('Name','Connectivity','NumberTitle','off',...
                                  'OuterPosition',[l b w h],'Color','white',... %insert s instead of [..] to get full screen
                                  'ToolBar','none');
        ...
        % Plot
        %imagesc(xPEB.Eq,[-1 1]); 
        %imagesc(xPEB.Eq,[-1.1705 1.1705]); 
        imagesc(xPEB.Eq, [-max(abs(Ep),[],'all'),max(abs(Ep),[],'all')]);
        colorbar;
        colormap(c);
        
        % Style
        axis square;     
        xlabel('From','FontSize',21,'FontName','Calibri','Fontweight','bold');
        ylabel('To','FontSize',21,'FontName','Calibri','Fontweight','bold');
        set(gca,'XAxisLocation','top','Tag','connectivity');
        set(gca,'Position',[0.2000 0.0726 0.6292 0.7500]);
        
        title_str = ['Connectivity: ' xPEB.fields{sel_field_idx}];
        if nu > 1
            if isequal(xPEB.fields(xPEB.sel_field_idx-1),{'D'})
            title_str = [title_str ' - ' xPEB.region_names{xPEB.sel_input}];
            else
            title_str = [title_str ' - ' xPEB.input_names{xPEB.sel_input}];
            end
        end
        
        title(title_str,'FontSize',42,...
            'FontName','Calibri');   
        
        if length(xPEB.region_names)> 8
            labelROIsize= 26;
        else
            labelROIsize= 32;
        end
        ...
        if size(xPEB.Eq,1) == size(xPEB.Eq,2) && ...
            ...
            set(gca,'YTickLabel',xPEB.region_names,'FontName','Calibri','FontSize',labelROIsize,...
                'YTick',1:length(xPEB.region_names),...
                'XTickLabel',xPEB.region_names,'FontName','Calibri','FontSize',labelROIsize,...
                'XTick',1:length(xPEB.region_names));
        elseif isequal(xPEB.fields(xPEB.sel_field_idx-1),{'C'})
            set(gca,'DataAspectRatioMode','manual');
            if length(xPEB.input_names)==2
                set(gca,'XTick',[1.25,1.75],'XLim',1:length(xPEB.input_names)); %must be changed if conditions are >2
            else
                set(gca,'XTick',1:length(xPEB.input_names));
            end
            set(gca,'YTickLabel',xPEB.region_names,'FontName','Calibri','FontSize',labelROIsize,...
                'YTick',1:length(xPEB.region_names),...
                'XTickLabel',xPEB.input_names,'FontName','Calibri','FontSize',labelROIsize);
        end
 
 if isequal(xPEB.fields(xPEB.sel_field_idx-1),{'C'})
         for side1=1:length(xPEB.input_names)
            for side2=1:length(xPEB.region_names)
               if xPEB.Eq(side2,side1) ~= 0
               text(side1,side2,num2str(round(xPEB.Eq(side2,side1),2)),'HorizontalAlignment','center','FontName','Calibri','Fontweight','bold','FontSize',34);
               elseif  xPEB.Eq(side2,side1) == 0 && EqOrig(side2,side1) == 0 && DCM{1,1}.c(side2,side1)==1
               text(side1,side2,'n.s.','HorizontalAlignment','center','FontName','Calibri','FontSize',32);                   
               elseif  xPEB.Eq(side2,side1) == 0 && EqOrig(side2,side1) ~= 0 && DCM{1,1}.c(side2,side1)==1
               text(side1,side2,num2str(round(EqOrig(side2,side1),2)),'HorizontalAlignment','center','FontName','Calibri','FontSize',32);                   
               end
            end 
         end
 else
     if size(xPEB.Eq,1) == size(xPEB.Eq,2) && ...
        size(xPEB.Eq,1) == length(xPEB.region_names)  
        for side1=1:length(xPEB.region_names)
            for side2=1:length(xPEB.region_names)
                if xPEB.Eq(side2,side1) ~= 0
               text(side1,side2,num2str(round(xPEB.Eq(side2,side1),2)),'HorizontalAlignment','center','FontName','Calibri','Fontweight','bold','FontSize',30);
                elseif isequal(xPEB.fields(xPEB.sel_field_idx-1),{'B'}) && xPEB.Eq(side2,side1) == 0 && EqOrig(side2,side1) == 0 && DCM{1,1}.b(side2,side1,sel_input)==1
               text(side1,side2,'n.s.','HorizontalAlignment','center','FontName','Calibri','FontSize',28);                   
                elseif isequal(xPEB.fields(xPEB.sel_field_idx-1),{'A'}) && xPEB.Eq(side2,side1) == 0 && EqOrig(side2,side1) == 0 && DCM{1,1}.a(side2,side1)==1
               text(side1,side2,'n.s.','HorizontalAlignment','center','FontName','Calibri','FontSize',28);    
               elseif isequal(xPEB.fields(xPEB.sel_field_idx-1),{'B'}) && xPEB.Eq(side2,side1) == 0 && EqOrig(side2,side1) ~= 0 && DCM{1,1}.b(side2,side1,sel_input)==1
               text(side1,side2,num2str(round(EqOrig(side2,side1),2)),'HorizontalAlignment','center','FontName','Calibri','FontSize',28);                   
                elseif isequal(xPEB.fields(xPEB.sel_field_idx-1),{'A'}) && xPEB.Eq(side2,side1) == 0 && EqOrig(side2,side1) ~= 0 && DCM{1,1}.a(side2,side1)==1
               text(side1,side2,num2str(round(EqOrig(side2,side1),2)),'HorizontalAlignment','center','FontName','Calibri','FontSize',28); 
                end
            end 
        ...
        p=gcf;
        p.PaperPositionMode = 'auto';
        title_str_val= matlab.lang.makeValidName(title_str);
        saveas(gcf,fullfile(cd,[title_str_val]),'bmp');
        %the following strings may be used to save with the correct name
        %providing project and dcm name.
        %fileName = [title_str_val,'_' project, '.DCM.', dcmName,'.bmp'];
        %saveas(gcf,fileName);
 end
